...
func (uis *UIServer) NewRouter() (*mux.Router, error) {
	...
	r.HandleFunc("/version/find/{project_id}/{revision}", uis.loadCtx(uis.versionFind)).Methods("GET")
	...
	r.HandleFunc("/json/task_timing/{project_id}/{build_variant}/{request}/{task_name}", uis.requireUser(uis.loadCtx(uis.taskTimingJSON))).Methods("GET")
	r.HandleFunc("/json/task_timing/{project_id}/{build_variant}/{request}", uis.requireUser(uis.loadCtx(uis.taskTimingJSON))).Methods("GET")
